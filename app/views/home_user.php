...
<body style="background-image: url('<?= base_url();?>public/resources/jenshin.gif'); background-size: cover; background-repeat: no-repeat; background-attachment: fixed;">
	...
	<form action="<?=site_url('home-user');?>" method="get" class="col-sm-4 float-end d-flex">
		<?php
		$q = '';
		if(isset($_GET['q'])) {
			$q = $_GET['q'];
    ...
    <h1 style="color: yellow;">Welcome Traveler <?=html_escape(get_role(get_user_id()));?>!</h1>
    <table>
        <thead>
            <tr>
                <th>ID</th>
    ...
    <div class="btn-links">
        <a href="<?= site_url('trash-user'); ?>" class="btn btn-bin">🗑️ Trash Bin</a>
